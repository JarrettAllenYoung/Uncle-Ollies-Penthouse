///////////////////////////
// HELPER GEOMETRY HELPERS
///////////////////////////

// Create a Box geometry (replacement for THREE.CubeGeometry)
function createBoxGeometry(width, height, depth, segments) {
    // Create a BufferGeometry using BoxGeometry, then convert it:
    var bufferGeom = new THREE.BoxGeometry(width, height, depth, segments || 1, segments || 1, segments || 1);
    var geom = new THREE.Geometry().fromBufferGeometry(bufferGeom);
    geom.mergeVertices();
    return geom;
  }
  
  // Create a Cylinder geometry and convert to Geometry
  ...
  var hero; // 3D object for our hero
  ...
  Hero = function() {
    ...
    // Ears – using Box geometry and modifying vertices
    var earGeom = createBoxGeometry(7, 18, 2, 1);
  ...
  Hero.prototype.jump = function(){
    ...
    TweenMax.to(this.earL.rotation, totalSpeed, { x: "+=0.3", ease: Back.easeOut });
    TweenMax.to(this.earR.rotation, totalSpeed, { x: "-=0.3", ease: Back.easeOut });
    
    TweenMax.to(this.pawFL.rotation, totalSpeed, { x: "+=0.7", ease: Back.easeOut });
    TweenMax.to(this.pawFR.rotation, totalSpeed, { x: "-=0.7", ease: Back.easeOut });
    TweenMax.to(this.pawBL.rotation, totalSpeed, { x: "+=0.7", ease: Back.easeOut });
    TweenMax.to(this.pawBR.rotation, totalSpeed, { x: "-=0.7", ease: Back.easeOut });
  ...
  Monster = function(){
    ...
    // Tail for Monster uses cylinder geometry; but for Hero we keep the tail defined earlier.
    // (If your Hero tail is defined above, do not override it here.)
  ...
  Hedgehog = function() {
    ...
    var eyeGeom = new THREE.CubeGeometry(1, 3, 3, 1);
    this.eyeL = new THREE.Mesh(eyeGeom, whiteMat);
    this.eyeL.position.set(2.2, 0.8, -0.5);
    this.eyeL.castShadow = true;
    this.head.add(this.eyeL);
    
    var irisGeom = new THREE.CubeGeometry(0.5, 1, 1, 1);
    this.iris = new THREE.Mesh(irisGeom, blackMat);
    ...
    var earGeom = createBoxGeometry(2, 2, 0.5, 1);
    ...
    var mouthGeom = createBoxGeometry(1, 1, 0.5, 1);
    this.mouth = new THREE.Mesh(mouthGeom, blackMat);
  ...
  Trunc = function(){
    ...
    var matTrunc = blackMat; // or choose randomly from mats if desired
    var nhSegments = 3;
    var nvSegments = 3;
    ...
    for (var i = 0; i < geom.vertices.length; i++){
      ...
      if (Math.random() > 0.5 && v.y > 10 && v.y < truncHeight - 10){
        ...
        var branchGeometry = createCylinderGeometry(thickness/2, thickness, h, 3, 1);
